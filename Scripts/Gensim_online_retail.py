...
df = load_csv(path="online_retail.csv")
...
csv_handler = CSVHandler('Gensim_online_retail_output.csv')

def tokenization_GENSIM(df):
    
    df['tokenized'] = df['Description'].apply(lambda row:([x for x in tokenize(row)]))
    return df['tokenized']
